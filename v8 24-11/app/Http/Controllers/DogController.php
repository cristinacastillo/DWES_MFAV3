...
class DogController extends Controller
{
    ...
    public function view(Request $req)
    {
        ...
        // 
        if($dog->status == 1):
            $nameUser = User::findName($req->input('id'));
        endif;
    ...
    public function add(Request $req)
    {
        ...
        Dog::create([
                    ...
                    'photo' => $namePhoto,
                    'created_at' => '2020-08-12 09:20:31',
                    ]);
    ...
    public function edit(Request $req){
        ...
        $redirect = redirect()->route('dog.info',['id' => $dog->idDog])->with('message', __('messages.update'));
    ...
    public function delete(Request $req){
            ...
            // check if the idDog and idUse are sent

            if(!$req->has('idDog') || !$req->has('idUse')) return redirect()->route('dogs.list');

            // check if the dog exists

            $dog = Dog::find($req->input('idDog'));

            // if the dog does not exist and the user is not admin it is redirected

            if(!$dog || ($req->has('idUse') != auth()->user()->idUse) ) return redirect()->route('dogs.list');

            // we delete the dog and redirect to admin

            $dog->delete();

            return redirect()->route('dogs.list')->with('message', __('messages.deleteDog'));
    ...
    public function search(Request $req){
       ...
       //Check if $req contains ajax or clear
        if($req->ajax() && $req->search != ""):
            ...
            $output= "";
            $data="";
            $data = DB::table('dog')->where('name','LIKE','%'.$req->search."%")->get();
            //$ruta = "{{ asset('/storage/dogs') }}/{{ $item->photo }}";
            ...
            if($data):
                foreach ($data as $item):
                   ...
                   // $output.='<p>'.$item->name.'</p>';
                    ...
                    $output.='<li class="cardsC_item">
                            <div class="cardC">
                                <div class="cardC_image">
                                    <img src="http://localhost/dwes/mfa/mfa/public/storage/dogs/'.$item->photo.'" class="imgCard" />
                                    
                                </div>
                                <div class="cardC_content">
                                    <a href="info?id='.$item->idDog.'"><button class="buttonC cardC_buttonC">' .$item->name.'</button></a>
                                </div>
                            </div>
                        </li>';
                endforeach;
